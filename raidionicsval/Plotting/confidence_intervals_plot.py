...
def compute_dice_confidence_intervals(folder, dices, best_overlap, postfix=""):
    folder = os.path.join(folder, 'Validation', 'Dice_CIs')
    ...
    try:
        ...
        fig, ax = plt.subplots()
        ax.set(xlabel='Dice score', ylabel='Number of patients', title='Dice confidence interval: [{:.2f}, {:.2f}]'.format(ci_pls[0, 0], ci_pls[1, 0]))
        ...
        # ax.legend()
        # plt.show()
        fig.savefig(os.path.join(folder, 'dice_confidence_intervals_all_tumors' + postfix + '.png'), dpi=300,
                    bbox_inches="tight")
        plt.close(fig)
        plt.clf()
        ...
        ax2.set(xlabel='Dice score', ylabel='Number of patients', title='Dice confidence interval: [{:.2f}, {:.2f}]'.format(ci_pls[0, 0], ci_pls[1, 0]))
        ...
        #ax2.legend()
        # plt.show()
        fig2.savefig(os.path.join(folder, 'dice_confidence_intervals_found_tumors' + postfix + '.png'),
